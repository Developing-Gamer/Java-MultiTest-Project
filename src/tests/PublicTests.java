...
public class PublicTests {
	...
	public void pub01TrueFalse() {
		...
		manager.addTrueFalseQuestion(10, 1,
				"Abstract classes cannot have constructors.", 2, false);
		manager.addTrueFalseQuestion(10, 2,
				"The equals method returns a boolean.", 4, true);
		manager.addTrueFalseQuestion(10, 3,
				"Identifiers can start with numbers.", 3, false);
		...
		answer.append("Exam score for " + studentName + " "
		...
		assertTrue(TestingSupport.isResultCorrect(testName, answer.toString(),
				true));
	}
	...
	public void pub03MultipleChoiceKey() {
		...
		points = 3;
		manager.addMultipleChoiceQuestion(10, 1, questionText, points,
				new String[] { "A", "C" });
		...
		points = 2;
		manager.addMultipleChoiceQuestion(10, 2, questionText, points,
				new String[] { "A", "B" });

		questionText = "Which of the following allow us to define a constant?\n";
		questionText += "A: abstract   B: equals   C: class   D: final ";
		points = 4;
		manager.addMultipleChoiceQuestion(10, 3, questionText, points,
				new String[] { "D" });
	...
	public void pub04MultipleChoice() {
		...
		String studentName = "Peterson,Rose";
		manager.addStudent(studentName);
		manager.answerMultipleChoiceQuestion(studentName, 10, 1,
				new String[] { "A", "C" });
		manager.answerMultipleChoiceQuestion(studentName, 10, 2,
				new String[] { "A", "B" });
		manager.answerMultipleChoiceQuestion(studentName, 10, 3,
				new String[] { "D" });
	...
	public void pub05FillInTheBlanksKey() {
		...
		points = 4;
		manager.addFillInTheBlanksQuestion(10, 1, questionText, points,
				new String[] { "static", "non-static" });

		questionText = "Name the first three types of primitive types discussed in class.";
		points = 6;
		manager.addFillInTheBlanksQuestion(10, 2, questionText, points,
	...
	public void pub06FillInTheBlanks() {
		...
		String studentName = "Peterson,Rose";
		manager.addStudent(studentName);
		manager.answerFillInTheBlanksQuestion(studentName, 10, 1,
				new String[] { "static", "non-static" });
		manager.answerFillInTheBlanksQuestion(studentName, 10, 2,
				new String[] { "int", "double", "boolean" });
		answer.append("Report for " + studentName + "\n"
				+ manager.getGradingReport(studentName, 10));
	...
	public void pub07CourseNumericLetterGrade() {
		...
		manager.addTrueFalseQuestion(1, 1,
				"Abstract classes cannot have constructors.", 10, false);
		manager.addTrueFalseQuestion(1, 2,
				"The equals method returns a boolean.", 20, true);

		String questionText = "Which of the following are valid ids?\n";
		questionText += "A: house   B: 674   C: age   D: &";
		points = 40;
		manager.addMultipleChoiceQuestion(1, 3, questionText, points,
				new String[] { "A", "C" });

		questionText = "Name the first three types of primitive types discussed in class.";
		points = 30;
		manager.addFillInTheBlanksQuestion(1, 4, questionText, points,
				new String[] { "int", "double", "boolean" });
		...
		manager.answerMultipleChoiceQuestion(studentName, 1, 3,
				new String[] { "A", "C" });
		manager.answerFillInTheBlanksQuestion(studentName, 1, 4,
				new String[] { "int", "double" });

		/* Second Exam */
		manager.addExam(2, "Midterm #2");
		manager.addTrueFalseQuestion(2, 1,
				"A break statement terminates a loop.", 10, true);
		manager.addTrueFalseQuestion(2, 2,
				"A class can implement multiple interfaces.", 50, true);
		manager.addTrueFalseQuestion(2, 3,
				"A class can extend multiple classes.", 40, false);
		...
		answer.append("Numeric grade for " + studentName + " "
				+ manager.getCourseNumericGrade(studentName));
		answer.append(
		...
		answer.append("\nCourse letter grade: "
				+ manager.getCourseLetterGrade(studentName));
	...
	public void pub08GetCourseGrades() {
		...
		manager.addExam(1, "Midterm #1");
		manager.addTrueFalseQuestion(1, 1,
				"Abstract classes cannot have constructors.", 35, false);
		manager.addTrueFalseQuestion(1, 2,
				"The equals method returns a boolean.", 15, true);
		manager.addTrueFalseQuestion(1, 3, "The hashCode method returns an int",
				50, true);
	...
	public void pub09MaxMinAverageScoreInExam() {
		...
		answer.append(
				"Average Score Midterm " + manager.getAverageScore(1) + "\n");
	...
	public void pub10MultipleExamsStudents() {
		...
		manager.addTrueFalseQuestion(examId, 1,
				"Java methods are examples of procedural abstractions.", 2,
				true);

		manager.addTrueFalseQuestion(examId, 2,
				"An inner class can only access public variables and methods of the enclosing class.",
				2, false);

		String questionText = "Which of the following allow us to define an abstract class?\n";
		questionText += "A: abstract   B: equals   C: class   D: final ";
		double points = 4;
		manager.addMultipleChoiceQuestion(examId, 3, questionText, points,
				new String[] { "A" });
		...
		manager.answerMultipleChoiceQuestion(laura, examId, 3,
				new String[] { "A" });
		...
		manager.answerMultipleChoiceQuestion(mike, examId, 3,
				new String[] { "A" });
		...
		manager.answerMultipleChoiceQuestion(john, examId, 3,
				new String[] { "A", "B", "C" });
		manager.answerFillInTheBlanksQuestion(john, examId, 4,
				new String[] { "private", "while" });

		/* Second Exam */
		examId = 2;
		manager.addExam(examId, "Midterm #2");
		manager.addTrueFalseQuestion(examId, 1,
				"The Comparable interface specifies a method called compareTo",
				2, true);
		manager.addTrueFalseQuestion(examId, 2,
				"The Comparator interface specifies a method called compare", 2,
				true);
		manager.addTrueFalseQuestion(examId, 3,
				"A static initialization block is executed when each object is created",
				4, false);
		...
		points = 8;
		manager.addMultipleChoiceQuestion(examId, 4, questionText, points,
				new String[] { "C" });

		questionText = "Which of the following are methods of the Object class?\n";
		questionText += "A: hashCode   B: equals   C: super   D: final ";
		points = 6;
		manager.addMultipleChoiceQuestion(examId, 5, questionText, points,
				new String[] { "A", "B" });
		...
		manager.answerMultipleChoiceQuestion(laura, examId, 4,
				new String[] { "C" });
		manager.answerMultipleChoiceQuestion(laura, examId, 5,
				new String[] { "A", "C" });

		manager.answerTrueFalseQuestion(mike, examId, 1, true);
		manager.answerTrueFalseQuestion(mike, examId, 2, true);
		manager.answerTrueFalseQuestion(mike, examId, 3, true);
		manager.answerMultipleChoiceQuestion(mike, examId, 4,
				new String[] { "C" });
		manager.answerMultipleChoiceQuestion(mike, examId, 5,
				new String[] { "A", "B" });
		...
		manager.answerMultipleChoiceQuestion(john, examId, 4,
				new String[] { "C" });
		manager.answerMultipleChoiceQuestion(john, examId, 5,
				new String[] { "A", "B" });
		...
		manager.addTrueFalseQuestion(examId, 1,
				"There are two types of exceptions: checked and unchecked.", 4,
				true);
		manager.addTrueFalseQuestion(examId, 2,
				"The traveling salesman problem is an example of an NP problem.",
				4, true);
		manager.addTrueFalseQuestion(examId, 3,
				"Array indexing takes O(n) time", 4, false);
		...
		manager.answerTrueFalseQuestion(john, examId, 3, false);
		manager.answerFillInTheBlanksQuestion(john, examId, 4,
				new String[] { "distributes values well" });
		...
		for (examId = 1; examId <= 3; examId++) {
			answer.append("Minimum for Exam # " + examId + " "
					+ manager.getMinScore(examId) + "\n");
			answer.append("Maximum for Exam # " + examId + " "
					+ manager.getMaxScore(examId) + "\n");
			answer.append("Average for Exam # " + examId + " "
					+ (int) manager.getAverageScore(examId) + "\n");
		}

		manager.setLetterGradesCutoffs(
				new String[] { "A+", "A", "B+", "B", "C", "D", "F" },
				new double[] { 95, 90, 85, 80, 70, 60, 0 });

		for (String student : list)
			answer.append("Letter Grade for " + student + " "
					+ manager.getCourseLetterGrade(student) + "\n");
	...
	public void pub11Serialization() {
		...
		SystemManager restoredManager = (SystemManager) manager
				.restoreManager(fileName);
	...
	public void pub12SerializationTwo() {
		...
		for (examId = 1; examId <= 3; examId++) {
			answer.append("Minimum for Exam # " + examId + " "
					+ restoredManager.getMinScore(examId) + "\n");
			answer.append("Maximum for Exam # " + examId + " "
					+ restoredManager.getMaxScore(examId) + "\n");
			answer.append("Average for Exam # " + examId + " "
					+ (int) restoredManager.getAverageScore(examId) + "\n");
		}

		restoredManager.setLetterGradesCutoffs(
				new String[] { "A+", "A", "B+", "B", "C", "D", "F" },
				new double[] { 95, 90, 85, 80, 70, 60, 0 });

		for (String student : list)
			answer.append("Letter Grade for " + student + " "
					+ restoredManager.getCourseLetterGrade(student) + "\n");
