...
public class FillBlanksQuestion implements QuestionInt, Serializable {
	...
	public void answerFillInTheBlanksQuestion(String studentName, int examId,
		...
		ArrayList<Object> infoList = queId.get(examId).get(questionNumber);
		String[] realAns = (String[]) infoList.get(2);
		...
		if (queId.containsKey(examId)
			...
			if (infoList != null && compareArrays(realAns, answer) != 0) {
				...
				value = (double) infoList.get(1) / (realAns.length)
						* compareArrays(realAns, answer);
	...
	private int compareArrays(String[] array1, String[] array2) {
		int count = 0;
